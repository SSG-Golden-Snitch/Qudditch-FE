...
const Drawing3d = (() => {
  const CAMERA_MAX_DEPTH = 1000

  let scene
  let camera
  let renderer
  let isSceneInit = false
  let isRendererInit = false
  ...
  const initScene = (width, height) => {
    ...
    const fov = 2 * Math.atan(diag / (2 * CAMERA_MAX_DEPTH)) * (180 / Math.PI)

    camera = new THREE.PerspectiveCamera(fov, width / height, 0.1, CAMERA_MAX_DEPTH)
  ...
  const resizeCamera = (width, height) => {
    if (isSceneInit && isRendererInit) {
      const diag = Math.sqrt(height * height + width * width)
      const fov = 2 * Math.atan(diag / (2 * CAMERA_MAX_DEPTH)) * (180 / Math.PI)
  ...
  const calculateDistance = (height) => {
  ...
  const createLabel = (category, score, color, canvasWidth, canvasHeight, mirrored, box) => {
    ...
    if (ctx) {
      ...
      mirrored
        ? ctx.roundRect(
            ...
            box.originY - 2,
            textSize.width + 8,
            textSize.fontBoundingBoxAscent + textSize.fontBoundingBoxDescent + 8,
          )
        : ctx.roundRect(
            box.originX - 2,
            box.originY - 2,
      ...
      mirrored
        ? ctx.fillText(
            name,
            canvasWidth - box.originX - box.width + 2,
            box.originY + textSize.fontBoundingBoxAscent + textSize.fontBoundingBoxDescent,
          )
        : ctx.fillText(
            name,
            box.originX + 2,
